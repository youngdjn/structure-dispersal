...
plot_perimeters <- st_read(data("regen/plot_layout/eiler/eiler_polygons.gpkg"),stringsAsFactors=FALSE) %>% st_transform(3310)
plot_perimeters$subarea = 1#plot_perimeters$Name#1:nrow(plot_perimeters)
...
plots = st_make_grid(plot_perimeters,cellsize=30,what="centers") %>% st_as_sf() %>% st_intersection(plot_perimeters)

plots_w_subunit = plots

st_write(plots_w_subunit, data("regen/plot_layout/eiler/eiler_points_01_prethin.gpkg"), delete_dsn = TRUE)


### Open the manually thinned plots
plots = st_read(data("regen/plot_layout/eiler/eiler_points_01_prethin.gpkg"))
...
st_write(plots %>% st_transform(4326), data("regen/plot_layout/forfield/eiler/eiler_plots_01.kml"), delete_dsn = TRUE)
...
write_csv(plots_geo,data("regen/plot_layout/forfield/eiler/eiler_plots_list.csv"))
