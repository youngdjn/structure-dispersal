...
fitted_2Dt = get_fitted_kernel(dataset_name = "crater-pipj-height-01",
                                      disp_mod = "2Dt",
                                      err_mod = "pois")

fitted_exppow = get_fitted_kernel(dataset_name = "crater-pipj-height-01",
                                          disp_mod = "exppow",
                                          err_mod = "pois")
...
seedl_at_plot_fast = predict_seedl_plot(samples = samples_median, tree_plot_dists = tree_plot_dists, overstory_tree_size = overstory_tree_size)
...
plan(multisession(workers=8))
plot_seedl_preds_fast = future_map_dfr(tree_dists_by_plot, predict_seedl_plot, samples = samples_median, overstory_tree_size = overstory_tree_size)
row.names(plot_seedl_preds_fast) = NULL
...
fitted_observed_plot_seedl_transf = fitted_observed_plot_seedl %>%
  mutate(obs = obs/0.09, # from seedl per plot to seedl per ha
         fit = fit*10000) # from seedl per sq m to seedl per ha

# Plot
ggplot(data=fitted_observed_plot_seedl_transf, aes(x=obs, y=fit)) +
