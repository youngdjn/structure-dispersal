...
#### Modified function that includes varying fecundity model ####
...
fit_stan_model_fecund = function(dataset_name, # which dataset to model (corresponding data files in datadir/prepped-for-stan/{dataset_name})
  ...
  ####### Run Stan model and save samples #######
  options(mc.cores = n_cores)
  model_file <- paste0("scripts/dispersal-modeling/dispersal-kernel-modeling/stan-models/stan-models-with-loglik-and-heightdiff/disp_", disp_mod, "_", err_mod, fecund_mod, ".stan")
