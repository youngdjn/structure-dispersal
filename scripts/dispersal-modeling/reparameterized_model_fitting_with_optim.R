...
# Differs from test_model_fitting_with_optim.R by reparameterizing a and k to "mean dispersal distance" and k to reduce correlations
...
calc_negloglik <- function(pars, n_overstory_trees, dist_vector, overstory_tree_size, pos, seedling_counts, seedling_plot_area) {
  ...
  k = pars[2]
  mean_dist = pars[3]
  # Reparameterize from mean_distance to a 
  a = mean_dist * (gamma(2 / k) / gamma(3 / k))
...
fit_model_optim <- function(startpars, n_overstory_trees = n_overstory_trees, dist_vector = dist_vector, overstory_tree_size = overstory_tree_size, pos = pos, seedling_counts = seedling_counts, seedling_plot_area = seedling_plot_area)
{
  ...
  b <- startpars$b
  k <- startpars$k
  mean_dist <- startpars$mean_dist
  pars.init <- c(b, k, mean_dist)
  
  fit <- optim(pars.init, method = "Nelder-Mead", control = list(maxit = 10000, trace = TRUE), calc_negloglik, n_overstory_trees = n_overstory_trees, dist_vector = dist_vector, overstory_tree_size = overstory_tree_size, pos = pos, seedling_counts = seedling_counts, seedling_plot_area = seedling_plot_area) # Use simulated annealing - slow but thorough
  ...
  estimates<-list(b=fit$par[1],
                  k=fit$par[2],
                  mean_dist=fit$par[3], 
                  a = fit$par[3] * (gamma(2) / gamma(3)))

  fv <- calc_mu(b = fit$par[1], k = fit$par[2], a = fit$par[3] * (gamma(2) / gamma(3)), n_overstory_trees = n_overstory_trees, dist_vector = dist_vector, overstory_tree_size = overstory_tree_size, pos = pos, seedling_plot_area = seedling_plot_area)
...
# Set and check starting parameters 
startpars = list(b = 1, k = 0.5, mean_dist = 40)
pars.init = c(startpars$b, startpars$k, startpars$mean_dist)
...
m = fit_model_optim(startpars, n_overstory_trees = disp_data$n_overstory_trees,
      dist_vector = disp_data$dist_vector, 
      overstory_tree_size = disp_data$overstory_tree_size, 
      pos = disp_data$pos,
      seedling_counts = disp_data$seedling_counts, 
...
bvals = 1
kvals = seq(0.5, 0.9, by = 0.025)
meanvals = seq(80, 140, by = 5)
parameter_test_set <- expand.grid(bvals, kvals, meanvals) 
names(parameter_test_set) = c("b", "k", "mean_dist")
...
ggplot(lik_surface_data, aes(x = mean_dist, y = k)) + 
  geom_tile(aes(fill = negloglikvals)) + 
  scale_fill_viridis_c() + 
  theme_minimal() + 
  labs(title = "Likelihood surface", x = "mean_dist", y = "k") +
