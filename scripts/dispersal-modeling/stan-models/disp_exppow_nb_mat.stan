...
//             // REMOVED: log_lik[i] = neg_binomial_2_lpmf(nseed[i] | mu_v[i], theta);
//             // REMOVED: nseed_sim[i] = poisson_rng(mu_v[i]);
//             // REMOVED: ll_sim[i] = poisson_lpmf(nseed_sim[i] | mu_v[i]);
//             // REMOVED: if (nseed_sim[i] > 0) nnz = nnz + 1;
//         }
//         // REMOVED: pval = sum(ll_sim) < sum(log_lik);
//         // REMOVED: totseeds = sum(nseed_sim);
//     }
//     
//     rmode = 0;
