...
functions {
    ...
    vector disp_prob(real k, real a, int n_overstory_trees, vector dist_vector) { 
      ...
      kernel_value = (k / (pi() * a)) * pow(1 + square(segment(dist_vector, pos[i], n_overstory_trees[i])) / a, -1-k); // apply 2Dt kernel to the distances for all trees associated with a given plot
      return(kernel_prob);
    }
}
...
transformed parameters {
    ...
    for(i in 1:n_seedling_plots){
          ...
          mu[i] = sum( disp_prob(k, a, n_overstory_trees[i], segment(dist_vector, pos[i], n_overstory_trees[i])) .*
