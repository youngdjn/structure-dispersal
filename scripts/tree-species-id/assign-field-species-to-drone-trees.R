...
source("join-predicted-and-observed-trees/lib/match-trees.R")

# Load field trees
trees_field = st_read(file.path(data_dir, "cross-site", "field-reference-trees", "stems_v4.gpkg")) |>
  st_transform(3310)

# Load field perims
perims_field = st_read(file.path(data_dir, "cross-site", "field-reference-trees", "plot_bounds_v4.gpkg")) |>
  st_transform(3310)
...
for (i in 1:nrow(stemmaps)) {
  ...
  trees_drone = st_read(file.path(datadir, "cross-site", "ttops", paste0(fire_name_foc, ".gpkg"))) |>
    st_transform(3310)
  ...
  # Load drone crowns
  crowns_drone = st_read(file.path(datadir, "cross-site", "crowns", paste0(fire_name_foc, ".gpkg"))) |>
    st_transform(3310) |>
    select(predicted_tree_id = treeID)
...
st_write(crowns_drone_w_field_data, file.path(data_dir, "cross-site", "crowns-w-field-labels", "crowns_drone_w_field_data.gpkg"), delete_dsn = TRUE)
