...
plot_perimeters <- st_read(data("regen/plot_layout/lassic/lassic_focal_2.gpkg"),stringsAsFactors=FALSE) %>% st_transform(3310)
plot_perimeters$subarea = 1:2#plot_perimeters$Name#1:nrow(plot_perimeters)
...
plots = st_make_grid(plot_perimeters,cellsize=40,what="centers") %>% st_as_sf() %>% st_intersection(plot_perimeters)

plots_w_subunit = plots

st_write(plots_w_subunit, data("regen/plot_layout/lassic/lassic_points_01_prethin.gpkg"), delete_dsn = TRUE)


### Open the manually thinned plots
plots = st_read(data("regen/plot_layout/lassic/lassic_points_01_thinned.gpkg"))
...
### Add a manually created set of plots
plots2 = st_read(data("regen/plot_layout/lassic/lassic_points_02_thinned.gpkg"))
geom = st_coordinates(plots2)
plots2 = plots2[!is.nan(geom[,1]),]
plots2$subarea = 3
plots2 = plots2 %>%
  mutate(Name = "lixo",
         Description = "lixo") %>%
  select(Name, Description,subarea)

plots = rbind(plots,plots2)
...
st_write(plots %>% st_transform(4326), data("regen/plot_layout/forfield/lassic/lassic_plots_01.kml"), delete_dsn = TRUE)
...
write_csv(plots_geo,data("regen/plot_layout/forfield/lassic/lassic_plots_list.csv"))
