...
plot_perimeters <- st_read(data("regen/plot_layout/chips/chips_perimeters2.gpkg"),stringsAsFactors=FALSE) %>% st_transform(3310)
...
plots = st_make_grid(plot_perimeters,cellsize=35,what="centers") %>% st_as_sf() %>% st_intersection(plot_perimeters)

plots_w_subunit = plots

st_write(plots_w_subunit, data("regen/plot_layout/chips/chips_points_01_prethin.gpkg"), delete_dsn = TRUE)


### Open the manually thinned plots
plots = st_read(data("regen/plot_layout/chips/chips_points_01_prethin.gpkg"))
...
st_write(plots %>% st_transform(4326), data("regen/plot_layout/forfield/chips/chips_plots_01.kml"), delete_dsn = TRUE)
...
write_csv(plots_geo,data("regen/plot_layout/forfield/chips/chips_plots_list.csv"))
